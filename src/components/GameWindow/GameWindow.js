...
const gameData = {
  ...
  reviews: {
    ...
    items: [
      { id: 1, user: "@user_generic", recommended: true, date: "November 23", text: "Its a shame this game has gotten the hate it has. Its actually pretty awesome. The combat is repetitive, yes, but so is Diablo??? There is an open world aspect that isnt like some stupid ubisoft game." },
      { id: 2, user: "@NonGenericUser", recommended: false, date: "November 25", text: "While definitely better than it was at launch, it still feels far too shallow and repetitive. The combat is fun at first, but you'll be fighting the same enemies in the same environments with unfun modifiers over, and over, and over again." },
      { id: 3, user: "@NonGenericUser", recommended: false, date: "November 25", text: "Bad game" },
      { id: 4, user: "@NonGenericUser", recommended: false, date: "November 25", text: "—{ Graphics }—\n☐ You forget what reality is\n☐ Beautiful\n☐ Good\n\n—{ Gameplay }—\n☐ Very good", hasShowMore: true },
      { id: 5, user: "@user_generic", recommended: true, date: "November 23", text: "> Watched Starship troopers before hand\n> Booted up Helldivers 2\n> First game I get absolutely curb stomped by a Bile Titan\n\n10/10 would play a third game" },
      { id: 6, user: "@user_generic", recommended: true, date: "November 23", text: "really really fun i love the warfare and free campaign" },
      { id: 7, user: "@user_generic", recommended: true, date: "November 23", text: "Great game. Extraction mode is best out there but needs some work on getting rid of cheaters." },
    ],
  },
};
  ...
  return (
    <div className="gp-page">
      ...
      <div className="gp-layout">
        ...
        <div className="gp-left">
          ...
          <div className="gp-main-image-wrap">
            <img
              ...
              src={selectedThumb === 0 ? gameData.mainImage : gameData.thumbnails[selectedThumb - 1]?.image}
              alt="main"
            />
          </div>
          ...
          <div className="gp-thumbs">
            ...
            {gameData.thumbnails.map((t, i) => (
              ...
              >
                ...
                {t.isVideo && <div className="gp-thumb-play"><PlayIcon /></div>}
              </div>
            ))}
          </div>
          ...
          <section className="gp-about">
            ...
            <button className="gp-show-more" onClick={() => setShowMoreAbout((v) => !v)}>
          ...
          <div id="container_achiements2">
            ...
            <div className="achievements_cards">
              {[
                { src: './card1.png', title: 'Hard achievement', sub: 'Completed by 0.5%' },
                { src: './card2.png', title: 'First steps', sub: 'Completed by 90%' },
                { src: './card3.png', title: 'Going forward', sub: 'Completed by 70%' },
                { src: './card4.png', title: 'Collect all the "Bags o...', sub: 'Completed by 7%' },
                { src: './card5.png', title: 'Complete your first ba...', sub: 'Completed by 60%' },
                { src: './card6.png', title: 'Final boss', sub: 'Completed by 10%' },
              ].map((c, i) => (
                <div className="achievements_card" key={i}>
                  <img src={c.src} alt={c.title} />
                  <p className="achievements_text1">{c.title}</p>
                  <p className="achievements_text2">{c.sub}</p>
                </div>
              ))}
            </div>
          ...
          <section className="gp-reviews">
            ...
            <div className="gp-reviews-sort">
              <span>Sort by:</span>
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="gp-sort-select">
                {gameData.reviews.sortOptions.map((o) => <option key={o}>{o}</option>)}
              </select>
            </div>
            <div className="gp-reviews-grid">
              {gameData.reviews.items.map((review) => (
                <div key={review.id} className="gp-review-card">
                    ...
                    <div className="gp-review-user">
                      ...
                      <div>
                        ...
                        <div className="gp-review-rec">
                          {review.recommended ? "This user recommends this game" : "This user does not recommend this game"}
                        </div>
                      </div>
                    </div>
                  ...
                  {review.hasShowMore && (
                    <button className="gp-show-more" onClick={() => toggleReviewExpand(review.id)}>
                  ...
                  <div className="gp-review-footer">
                    ...
                    <button className="gp-thumb-btn gp-thumb-down"><ThumbIcon /></button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* ── Правый сайдбар (на мобиле — сверху через order: -1) ── */}
